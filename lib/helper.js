...
let logger = (data, error) => {
  if (error) {
    console.log(chalk.bgYellow(chalk.red("Error!")));
    console.log(chalk.bgRed(chalk.white(`${JSON.stringify(data, null, 2)}`)));
  } else {
    console.log(chalk.bgWhite(chalk.green("Success!")));
    console.log(chalk.bgGreen(chalk.white(`${JSON.stringify(data, null, 2)}`)));
  }
};
